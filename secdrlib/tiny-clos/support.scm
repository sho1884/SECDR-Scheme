...
(define gsort
  (case what-scheme-implementation
    ...
    ((secdr)(lambda (predicate list) (qsort list predicate)))
    ((r4rs)(begin (require 'sort)
                   (lambda (predicate list) (sort list predicate))))))
...
(define remove-duplicates
    (lambda (list)
      (let loop ((result-so-far '())
	   ...
	   (if (null? remaining)
	       result-so-far
	       (if (not (memq (car remaining) result-so-far))
